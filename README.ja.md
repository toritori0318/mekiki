...
| コマンド | 何をするか |
|---|---|
| `mekiki lint` | 25の番号付き規則で資産を検査する。散文に置き去りにされた計算処理、宣言されているだけで起動されないガード、同じ依頼を2つのスキルが取り合っている状態まで。`--changed` で PR が触ったスキルだけに報告を絞れる。`--jev` で正規表現には決められない 5 つの問いを較正済み分類器に任意で問える |
