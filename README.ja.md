...
`skills/auditing-skill-corpus/` は、この監査を**正しい順序で**回させるための Agent Skill です。
指摘を1件も読む前に baseline を決める、継承した積み残しではなく差分でゲートする、規則が
名指しした層で直す（引っかかった文を消して済ませない）、ガード名は**捏造せず訊く**。
エージェントがスキルを探す場所にコピーしてください:

```bash
cp -r skills/auditing-skill-corpus ~/.claude/skills/
```

このスキル自身が、教えている規約に従っています —— `mekiki lint skills` は無指摘、Tier は
T3、そしてその状態を守るテストが本リポジトリにあります。
