...
`skills/auditing-skill-corpus/` は、このツールを動かすための Agent Skill である。存在理由は、
mekiki が出す指摘文それ自体は読めば分かる一方で、**作業の順序**は分からないから。そして
エージェントが間違えるのは順序のほうである。継承した資産にいきなり lint をかけて編集を始める
（最初の一手は baseline を決めること）、warn を作業リストとして扱う、散文ヒューリスティックの
指摘を「引っかかった文を消す」ことで直す、ガードが未設定のときにガード名を捏造する。

| パス | 内容 |
|---|---|
| `SKILL.md` | 順序そのもの: baseline が先、問いに対応するコマンド、出力の仕分け、決めずに訊くべきこと |
| `references/rule-playbook.md` | 規則ごとの修復手順。規則番号順ではなく「修復アクション」別に束ねる |
| `evals/evals.json` / `evals/eval_queries.json` | 公式2形式。3ケースと20クエリ（発火10・近傍スキルを狙った非発火10） |
...
それを検査する。規約を満たすために書かれた唯一のスキルが満たせない新規則は、出荷前に見直す
べき規則である。
